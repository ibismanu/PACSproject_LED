...
# TODO: capire quali for sono rimpiazzabili
...
class RungeKutta(DataGen):
    ...
    def __init__(self, T, dt, u0, but_A, but_b, but_c, M=None, A=None, F=None, f=None, eqtype=""):
        ...
        is_valid_c = sum(np.abs(np.array(
            [np.sum(row) for row in but_A])-but_c)) < 2*len(but_c)*sys.float_info.epsilon
        len_b = len(but_b)
        size_A = but_A.shape
...
class RK_explicit(RungeKutta):
    def __init__(self, T, dt, u0, but_A, but_b, but_c, M=None, A=None, F=None, f=None, eqtype=""):
        super().__init__(T=T, dt=dt, u0=u0, but_A=but_A, but_b=but_b,
                         but_c=but_c, M=M, A=A, F=F, f=f, eqtype=eqtype)
...
class RK_implicit(RungeKutta):
    def __init__(self, T, dt, u0, but_A, but_b, but_c, M=None, A=None, F=None, f=None, eqtype=""):
        ...
        if is_expl_A:
            warnings.warn(
                "implicit method called but explicit butcher array given")
        elif is_semi_A:
            warnings.warn(
                "implicit method called but semi-implicit butcher array given")
        self.but_A = but_A
...
class RK4(RK_explicit):
    def __init__(self, T, dt, u0, M=None, A=None, F=None, f=None, eqtype=""):
        but_A = np.array([[0, 0, 0, 0], [1, 0, 0, 0],
                         [-1, 3, 0, 0], [3, -3, 3, 0]], dtype=float)/3
    ...
    def generateODE(self):
        ...
        for k in range(self.numIT-1):
            rhs = self.u[:, k]+self.dt * \
                (1-self.theta)*self.f(self.u[:, k], self.times[k])
            ...
            self.u[:, k +
                   1] = sp.optimize.newton(g, x0=self.u[:, k], tol=size*1e-2)
        # return u,times
...
class Multistep(DataGen):
    def __init__(self, T, dt, u0, mul_a, mul_b, f=None, M=None, A=None, F=None, eqtype=""):
        super().__init__(T=T, dt=dt, u0=u0, M=M, A=A, F=F, f=f, eqtype=eqtype)
        self.mul_a = mul_a
        self.mul_b = mul_b
...
class Adams_Bashford(Multistep):
    def __init__(self, T, dt, u0, p, f=None, M=None, A=None, F=None, eqtype=""):

        def g(v, j): return 0.5 * \
            np.prod(
                np.array([(0.5*(v+1))+i for i in range(p-1)])) / ((0.5*(v+1))+j)

        def integral(j):
            deg = (p+1)//2

            nodes, weights = np.polynomial.legendre.leggauss(deg=deg)
            return np.sum(np.array([weights[i] * f(nodes[i]) for i in range(deg)]))
        ...
        super().__init__(T=T, dt=dt, u0=u0, mul_a=mul_a,
                         mul_b=mul_b, M=M, A=A, F=F, f=f, eqtype=eqtype)

    def generate(self):
        return super().generate()


class Adams_Moulton(Multistep):
    def __init__(self, T, dt, u0, mul_b, f=None, M=None, A=None, F=None, eqtype=""):

        mul_a = np.zeros(len(mul_b))
...
class BDF(Multistep):
    def __init__(self, T, dt, u0, mul_a, f=None, M=None, A=None, F=None, eqtype=""):

        mul_b = np.zeros(len(mul_a))

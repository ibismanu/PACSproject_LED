...
class MitchellSchaeffer(DataGen):
    ...
    def generate_sample(self, args):
        ...
        if x0 is None:
            x0 = (np.random.uniform(0,self.grid_size[0]),np.random.uniform(0,self.grid_size[1]))
        ...
        for x in range(self.grid_size[0]):
            for y in range(self.grid_size[1]):
                ...
                self.sample[:, x, y] = solver.u.transpose()               

        filename = f'sample_{sample_id}.npy'
        np.save(filename, self.sample)

    def generate_dataset_parallel(self, num_samples, num_processes, x0=None, plot=False):
